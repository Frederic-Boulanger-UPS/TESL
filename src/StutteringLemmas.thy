...
lemma counted_ticks_sub:
  assumes \<open>dilating f sub r\<close>
    shows \<open>counted_ticks sub c n m d = counted_ticks r c (f n) (f m) d\<close>
proof -
  have 1:\<open>n \<le> m = ((f n) \<le> (f m))\<close> using assms
    by (simp add: dilating_fun_def dilating_def strict_mono_less_eq)
  have 2:\<open>(run_tick_count sub c m = run_tick_count sub c n + d) = (run_tick_count r c (f m) = run_tick_count r c (f n) + d)\<close>
    using run_tick_count_sub[OF assms] by simp
  have 3:\<open>(\<nexists>m'. (n \<le> m') \<and> (m' < m) \<and> run_tick_count sub c m' = run_tick_count sub c n + d)
      = (\<nexists>m'. ((f n) \<le> m') \<and> (m' < (f m)) \<and> run_tick_count r c m' = run_tick_count r c (f n) + d)\<close>
    sorry
